...
// Set the limit on the number of products, sales and companies
...
function generateCompanies(count) {
  ...
  return function(__ignore, callback) {
    ...
    return async.parallel(createFns, callback);
  };
...
function generateProducts(count) {
  ...
  return function(companies, callback) {
...
function generateSales(count) {
  ...
  return function(products, callback) {
    var productIds = products.map(function (c) {
      return c.id;
    });
    var createFns = _.range(count).map(function () {
